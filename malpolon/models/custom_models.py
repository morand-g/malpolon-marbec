...
class MultiModalModel(nn.Module):
        ...
        if not self.monomodal and not self.mae_decoder:
            ...
            if aggregator == 'Linear':
                ...
                self.aggregator_model =  torchvision.ops.MLP(in_channels = sum([x.in_features for x in linears]),
                                                        hidden_channels = [max(linears[0].out_features // 4, 8), linears[0].out_features],
                                                        dropout = 0.3)
    ...
    def edit_final_layer(self, new_species_num = None, cp = None):
        ...
        if new_species_num is None:
            new_species_num = next(reversed(cp['state_dict'].values())).shape[0] // self.num_bins
