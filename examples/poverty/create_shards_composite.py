...
Example usage::
    ...
    create_shards(
        source_dir="path/to/tiffs",          # country/year/cluster.tiff
...
def _process_sample(source_dir, country, year, cluster_id, iwi, image_size):
    """Read and preprocess one composite raster.

    Returns:
        (key, tile_bytes, label_bytes, band_sum, band_sq_sum, n_pixels)
        or None if the raster is missing.

    band_sum and band_sq_sum are (6,) float64 arrays in stored uint16 units,
    accumulated over all pixels.
    """
    source_dir = Path(source_dir)
    key = f"{country}_{year}_{cluster_id}"

    tiff_path = source_dir / country / year / f"{cluster_id}.tif"

    if not tiff_path.exists():
        print(f"Missing raster: {tiff_path}")
        return None

    tile = _read_tile(str(tiff_path), image_size)
    # Expected shape: (6, image_size, image_size), dtype uint16
...
def create_shards(
    ...
    Per-sample tar entries:
        {key}.input  -- raw uint16 bytes for (6, image_size, image_size)
    ...
    ----------
    source_dir : str
        Directory with Landsat TIFFs (country/year/cluster.tiff).
    source_csv : str
